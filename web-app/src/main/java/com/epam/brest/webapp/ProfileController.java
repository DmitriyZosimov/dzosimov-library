...
public class ProfileController {
  ...
  @Autowired
  public ProfileController(ReaderService readerService, BookService bookService,
  ...
  public String getProfile(Model model, HttpSession session, HttpServletRequest request) {
    ...
    ReaderSample readerSample = readerService.findReaderById(card);
  ...
  /**
   * Goto a reader page with model attribute "isNew" is false and with found reader by
   * identification.
   *
   * @param model   used for rendering views
   * @param session {@link HttpSession} is necessary to get reader id
  ...
  public String editProfile(Model model, HttpSession session, HttpServletRequest request) {
    ...
    Integer card = (Integer) session.getAttribute("libraryCard");
    ReaderSample readerSample = readerService.findReaderByIdWithoutBooks(card);
    model.addAttribute("readerSample", readerSample);
    LOGGER.debug("READER: {}", readerSample);
   ...
   * @param bindingResult      used for error registration capabilities, allowing for a {@link
   *                           javax.validation.Validator} to be applied.
   * @param model              used for rendering views
   * @param request            HttpServletRequest is necessary for getting a current locale
   * @param redirectAttributes used for adding attribute to redirect page
  ...
  public String editProfile(@Valid ReaderSample readerSample, BindingResult bindingResult,
    ...
    String messageCode;
    if (readerService.editReader(readerSample)) {
      messageCode = "message.reader.edit.good";
    } else {
      messageCode = "message.reader.edit.bad";
    }
    String message = messageSource
        .getMessage(messageCode, null, localeResolver.resolveLocale(request));
    LOGGER.info(message);
    redirectAttributes.addAttribute("resultMessage", message);
  ...
  public String removeProfile(Model model, HttpSession session, HttpServletRequest request,
    ...
    Integer card = (Integer) session.getAttribute("libraryCard");
    String messageCode;
    if (readerService.changeReaderToNoActive(card)) {
  ...
  public String restoreProfile(@PathVariable("card") @Min(1) Integer card,
    ...
    String messageCode;
    Integer[] objsForMessage = null;
    if (readerService.changeReaderToActive(card)) {
  ...
  public String removeBookFromReader(@PathVariable("book") @Min(1) Integer bookId,
      Model model, HttpSession session) {
    LOGGER.info("POST /profile/book/delete/bookId={}", bookId);
    Integer readerId = (Integer) session.getAttribute("libraryCard");
    bookService.removeBookFromReader(bookId, readerId);
    return "redirect:/profile";
  }

  @GetMapping(value = "/readers/search")
  public String searchReaderByDate(SearchReaderSample searchReaderSample,
